# 🏗️ Guía de Arquitectura: Ecosistema FinOps Multi-Agente v2.1
...
```mermaid
graph TD
    Trigger[Cloud Scheduler / Webhook] --> API[FastAPI on Cloud Run]
    API --> Orchestrator[Orquestador Maestro]
    
    subgraph "Agentes de Inteligencia (ADK + Gemini)"
...
Cada agente en `src/agents/` es una instancia de `google.adk.Agent`. Utilizan **Gemini 1.5** vía Vertex AI para:
