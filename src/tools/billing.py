...
def get_top_services_delta(project_id: str) -> List[str]:
    """
    FIX: Implementación real con query a BigQuery.
    Retorna los top 3 servicios GCP con mayor incremento de costo en el mes actual
    vs. el mes anterior, para un proyecto específico.

    Útil para contextualizar en el reporte de anomalías qué servicio está causando el alza.
    """
    client = get_bq_client()
    table_id = "pisa-proyecto-de-monitoreo.billing_PiSA_corpcabUSD.gcp_billing_export_v1_018D25_BD21E0_DFDB79"
    ...
    WITH current_month AS (
      SELECT
        ...
        SUM(cost) + SUM(IFNULL((SELECT SUM(c.amount) FROM UNNEST(credits) c), 0)) AS costo_actual
      FROM `{table_id}`
      WHERE invoice.month = FORMAT_DATE('%Y%m', CURRENT_DATE())
    ...
    previous_month AS (
      SELECT
        service.description AS servicio,
        SUM(cost) + SUM(IFNULL((SELECT SUM(c.amount) FROM UNNEST(credits) c), 0)) AS costo_anterior
      FROM `{table_id}`
      WHERE invoice.month = FORMAT_DATE('%Y%m', DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH))
        AND project.id = @project_id
      GROUP BY servicio
    ...
    SELECT
      c.servicio,
      c.costo_actual,
      COALESCE(p.costo_anterior, 0) AS costo_anterior,
      (c.costo_actual - COALESCE(p.costo_anterior, 0)) AS delta_usd
    FROM current_month c
    LEFT JOIN previous_month p USING (servicio)
    ...
    try:
        ...
        return [row.servicio for row in results if row.servicio]
    except Exception as e:
        print(f"[!] Error obteniendo top services delta para {project_id}: {e}")
        return []
